...
## 1) Filas duplicadas por id, contiguas (para inspección rápida)
dup_rows <- duplicated(df$id) | duplicated(df$id, fromLast = TRUE)
dups_df  <- df[dup_rows, ][order(df$id[dup_rows]), ]
head(dups_df, 20)

## 2) Columnas a comparar (excluye 1:3, sin borrar del df)
cols_idx <- setdiff(seq_along(df), c(1:3, 5))

## 3) Comparación por id (qué columnas difieren entre duplicados)
#    split evita tener que crear dup_ids y subsetting en bucle
...
## 4) Resumen: n° de variables distintas y cuáles son
...
## 5) Distribución y combinaciones
...
2) Some observations have different information for some variables: check those carefully
```{r}
# Case 1
## ids cuyo diff_vars == "None"
ids_none <- summary_df$id[summary_df$diff_vars == "None"]

## mantener SOLO la primera fila de cada id en ese subconjunto,
## y dejar intacto el resto de ids
keep_idx <- ifelse(df$id %in% ids_none, !duplicated(df$id), TRUE)

## df actualizado (solo deduplicado para los casos "None")
df <- df[keep_idx, ]

## comprobación
sum(duplicated(df$id) & df$id %in% ids_none)  # debe ser 0

```
```{r}
# Case 2
## resumen que nos interesa (excluye "None")
summary_rest <- summary_df[summary_df$diff_vars != "None", ]

## ver cuántos ids tiene cada combinación
sort(table(summary_rest$diff_vars), decreasing = TRUE)

## helper: función para ver todas las filas en df de una combinación concreta
ver_comb <- function(combo_str) {
...
```{r}
# los 12 casos que difieren en "description"
ver_comb("description") 

# los 7 casos: "event, time, price, pricePerSquareFoot"
ver_comb("event, time, price, pricePerSquareFoot")

# un combo largo de ejemplo
ver_comb("time, price, pricePerSquareFoot, description, garageSpaces, hasGarage")
# el precio ha bajado con el tiempo pero lo raro es que ha cambiado el garage (ahora si q tiene)
