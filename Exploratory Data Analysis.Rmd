...
We remove the observation with 14 bedrooms, as it is an extreme observation that could negatively affect the model.

```{r}
# Eliminate observation with 14 bedrooms
df <- df[df$bedrooms != 14, ]
```

As can be seen in the histogram, some observations have 0 bedrooms. Just as `bathrooms` variable, these values are replaced with NA to indicates missing data and then later we will impute the variable using the **MICE** algorithm. 
...
```
Since `event` primarily indicates listing status and not intrinsic property characteristics, it may not contribute significantly to price prediction. It's strongly imbalanced and it could be removed later.
...
Variable `year.built` is also removed, and we will consider the `n.yearBuilt`variable (years since house has been built).

```{r}
# Columns removed after initial analysis
df2 <- df[,-c(1:12, 15:24, 26, 29, 37, 39, 43)]
```
...
#### Variable `bathrooms`: cat
Bathrooms variable is a discrete numerical variable. To simplify the analysis and prepare it for modeling, we created a categorical variable with three levels: 1–2, 3–4, and 5 or more bathrooms.

```{r}
# convert to factor
df3$f.bathrooms <- cut(df3$bathrooms,
                      breaks = c(0, 2, 4, Inf),
                      labels = c("1-2", "3-4", "5 or more"),
                      right = TRUE)

barplot(table(df3$f.bathrooms), main = "Barplot of f.bathrooms", col="lightblue")
```

#### Variable `bedrooms`: cat
Bedrooms variable is a discrete numerical variable. To simplify the analysis and prepare it for modeling, we created a categorical variable with 3 levels: 1–2, 3–4, 5 or more bedrooms.

```{r}
# Convert to factor
df3$f.bedrooms <- cut(df3$bedrooms,
                      breaks = c(0, 2, 4, Inf),
                      labels = c("1-2", "3-4", "5 or more"),
                      right = TRUE)

barplot(table(df3$f.bedrooms), main = "Barplot of f.bedrooms", col="lightblue")
```
...
```{r}
# Profiling: Analysis with price as numerical
library(FactoMineR)
...
```{r}
# Profiling: Analysis with price as a factor
...
boxplot(price ~ f.bathrooms, data = df4,
        main = "Price ~ f.bathrooms",
        col = rainbow(length(unique(df4$f.bathrooms))))
boxplot(price ~ f.bedrooms, data = df4,
        main = "Price ~ f.bedrooms",
        col = rainbow(length(unique(df4$f.bedrooms))))
```
