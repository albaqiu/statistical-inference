...
```
**Problem:** 
After an exploratory analysis of the duplicates, it is observed that some observations have the same information for all variables but different `countyId`. Since the variable `county` shows the same value for duplicated instances, `countyId` (5) will be removed due to the inconsistencies. 

However, that is not the only case of duplicates in this df.
...
We manage the duplicates separating them in 2 cases:
...
In order to thoroughly treat the duplicates, the individual observations have been analysed for each combination of differences they have (for example, observations with different description, different value for garageSpaces...)
...
```
The variable `pricePerSquareFoot` presents a strong right-skew with a median around 153, while the maximum reaches an unusually extreme value (over 200,000). This value is considered a data entry error and will be removed.

The IQR analysis detects 258 mild outliers and 51 extreme outliers, confirming the presence of several abnormally high observations. These values likely result from data entry errors or exceptional listings and will require treatment to prevent them from disproportionately influencing the model.
...
# We calculate the outliers again
...
```
First, the variable was treated, as some city names were duplicated due to abbreviations, spelling variations, or inconsistent capitalization, in hopes of being able to use the information provided by this variable
...
length(unique(df$city))
```
But even after these corrections, the factor has too many levels, which are not easily regrouped. Therefore, this variable is removed from the analysis.
...
# convert to factor
df$levels <- factor(df$levels, labels = c("one", "two", "three or more"))
...
vars <- c(  "bathrooms", "bedrooms", "levels", "livingArea", "price", "pricePerSquareFoot", 
            "garageSpaces", "hasGarage", "parking", "pool", "spa", "homeType", 
            "isNewConstruction", "n.yearBuilt")
...
```
To validate the results of the imputation, we compare the quantiles before and after to see if they have changed. 
...
The univariate outliers have been detected in the previous section for each of the numeric variables. 
The cases where it was necessary to remove them have already been handled. 
The others will be stored in additional variables `mild_uot`and `extr_uot`.
```{r, include=FALSE}
...
df3$mild_uout <- dfout$mild_uout
df3$extr_uout <- dfout$extr_uout
...
```{r, results='hide'}
ll <- which((res.mout$md>res.mout$cutoff) & (res.mout$rd>res.mout$cutoff)) # observations with md and rd > cutoff
```
...
- Moderate association:  
  `n.yearBuilt` (~0.24) shows newer constructions are slightly more expensive.  
...
- Top numeric discriminators:  
  `price` (trivial, derived), `livingArea`, `bathrooms`, `pricePerSquareFoot`, `bedrooms`, `n.yearBuilt`. 
...
```{r, include=FALSE}
# Boxplot
boxplot(price ~ levels, data = df4,
        main = "Price ~ Levels",
        col = rainbow(length(unique(df4$levels))))
boxplot(price ~ garageSpaces, data = df4,
        main = "Price ~ garageSpaces",
        col = rainbow(length(unique(df4$garageSpaces))))
boxplot(price ~ hasGarage, data = df4,
        main = "Price ~ hasGarage",
        col = rainbow(length(unique(df$hasGarage))))
boxplot(price ~ parking, data = df,
        main = "Price ~ parking",
        col = rainbow(length(unique(df4$parking))))
boxplot(price ~ pool, data = df4,
        main = "Price ~ pool",
        col = rainbow(length(unique(df4$pool))))
boxplot(price ~ spa, data = df4,
        main = "Price ~ spa",
        col = rainbow(length(unique(df4$spa))))
boxplot(price ~ homeType, data = df4,
        main = "Price ~ homeType",
        col = rainbow(length(unique(df4$homeType))))
boxplot(price ~ isNewConstruction, data = df4,
        main = "Price ~ isNewConstruction",
        col = rainbow(length(unique(df4$isNewConstruction))))
boxplot(price ~ f.yearBuilt, data = df4,
        main = "Price ~ f.yearBuilt",
        col = rainbow(length(unique(df4$f.yearBuilt))))
```
